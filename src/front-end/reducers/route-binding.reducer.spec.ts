...
describe('routeBinding reducer', () => {
  it('should given reducerA return a reducerB that returns the result of ' +
    'reducerA', () => {
    const reducerA = (state, action) => 5;
  ...
  it('should given a matching action compute a new state using the ' +
    ...
    const reducerB = routeBindingOn({
      ...
      otherAction1: {
        newState: (path, state) => Object
          .assign({}, state, { sValue: state.sValue + ' ' + path }),
        fromPayload: noop,
      },
      otherAction2: {
        newState: (path, state) => Object
    ...
    expect(newState.other === 'world hello' ||
  ...
  describe('newState_LOCATION_CHANGE', () => {
    it('should update the pathname of a given react\.locationBeforeTransitions',
      () => {
        const state = { react: { locationBeforeTransitions: {
          pathname: 12,
        } } };
    ...
    it('should create a locationsBeforeTransitions sub object if it does ' +
